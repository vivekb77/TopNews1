...
router.post('/PostOnInsta', async (req, res) => {
    ...
    try {
        ...
        if (currentTimeNZT >= startTimeNZT && currentTimeNZT <= endTimeNZT) {
            ...
            console.log("Skipping posting on Insta: " + currentTimeNZT);
            return res.json({ status: 'ok', message: "Skipped posting on Insta: " + currentTimeNZT })
        }
    } catch (err) {
        console.log(err);
...
async function getImageUrl() {
    ...
    try {
        ...
        if (imageUploadResponse.status === 200 && imageUploadResponse.data.success === true) {
            image_id = imageUploadResponse.data.response.id
            image_url = `https://newsexpress.imageresizer.io/${image_id}?size=1080x1800&format=webp&qualty=80`;
            console.log("Image uploaded to image resizer");
            //now post insta post and story, post on even hour, story on odd hour
            var currentHour = new Date().getHours();
            if (currentHour % 2 === 0) {
                await PostAPost(image_url, post_caption);
            } else {
                await PostAStory(image_url);
            }
            // await PostAReel();
...
async function PostAPost(image_url, post_caption) {
    try {
        //post upload
...
async function PostAStory(image_url) {
    try {
        //story upload
        const response = await axios.post(
