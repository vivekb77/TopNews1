...
//get news articles
router.post('/GetNewsForWorld', async (req, res) => {
    ...
    const currentDate = moment().tz(timeZone).startOf('day').toDate(); // Get the current date in the specified time zone
        ...
        if(topictopulltweets == "PullAllNews"){
             AITweets = await NewsDataWorld.find({
                displayOnFE: true,
...
router.post('/dateTimeOfLastPullWorld', async (req, res) => {
try {
    const dateTimeOfLastPull = await DateTimeOfLastPullModel.find({
        _id: "64b7bd95181d90534a16cb5a"
    })
    const inputDate = new Date(dateTimeOfLastPull[0].dateTimeOfLastPull_World);
                const options = { 
                    day: '2-digit', 
                    month: 'short',
                    // year: '2-digit', 
